...
**Why:** Personica의 핵심 가치는 속도나 편의성이 아니라 **페르소나 리얼리티** — 시뮬레이션의 깊이와 충실도. 5-Layer 엔진이 만드는 사람 같은 반응이 해자.
...
**How to apply:** 다중 세션 메모리 설계 시 이 맥락 참고. Generative Agents → Graph RAG 확장 경로를 염두에 둘 것. 단, 핵심 해자는 5-Layer 엔진의 페르소나 리얼리티이고, 다중 세션은 추가 해자.

### Personica 제품 핵심 정의
> Personica = 합성 페르소나 시뮬레이션 플랫폼. 핵심 해자는 페르소나 리얼리티(5-Layer 엔진). 적용 범위 확장 (2026-03-21)
...
**핵심 해자 = 페르소나 리얼리티**: 5-Layer 시뮬레이션 엔진이 핵심 IP
...
- 핵심 해자: 5-Layer 엔진의 페르소나 리얼리티 (감정·인지·의사결정 시뮬레이션)
...
**How to apply:** 전략 문서, 피치 자료에서 "합성 페르소나 시뮬레이션 플랫폼"으로 포지셔닝. 경쟁사들과 동일한 영역에서 5-Layer 엔진의 깊이 차별화 + 제품 체험 모드의 추가 차별화.

### 예창패 사업계획서 작성 가이드라인
> 예창패 PSST 요구사항 + 합격 사례 분석 + VC 피드백을 종합한 섹션별 작성 가이드. 사업계획서 작성 세션에서 반드시 참조.

예창패 사업계획서 작성 가이드라인 문서:
- `docs/strategy/bizplan-writing-guide.md`
- `docs/strategy/bizplan-preparation-guide.md`

**핵심 내용:**
- 드라마 아크 구조: 공감→긴장→반전→증거→비전
- PSST 섹션별 작성법 (필수 시각자료, 드라마 구조, Do/Don't)
- 분량 가이드: P(30~35%) > S1(30~35%) > S2(20%) > T(10~15%)
- 시각자료 체크리스트 15개 항목
- 세션별 작업 순서 (6세션으로 분할)

**이럴 때 참고:**
- 사업계획서 섹션 작성 시 구조/톤/시각자료 수준 확인
- 새 세션에서 이어서 작성할 때 전체 맥락 파악
- 시각자료 어떤 유형을 써야 할지 결정할 때

**관련 참고 자료:**
- **사업계획서 틀(템플릿)**: `docs/예창패 실제 제출용/bizplan-template.md` — 섹션별 작성 대기 상태, 필요 시각자료 18개 목록 포함
