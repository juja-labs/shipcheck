# Blok Deep Dive: ShipCheck의 가장 직접적 경쟁자 심층 분석

**Date**: 2026-03-16
**Version**: 1.0
**목적**: Blok이 ShipCheck의 비전을 이미 실현했는지, ShipCheck만의 차별화 여지가 있는지 팩트 기반으로 판단
...
이는 ShipCheck의 접근 방식과 **근본적으로 다르다**:
- **ShipCheck**: Playwright로 실제 프로덕션 MVP를 브라우저에서 조작. 실제 DOM, 실제 인터랙션, 실제 응답시간 경험.
...
**핵심 차이**: Blok은 **기존 제품 데이터가 있는 팀**을 타겟한다. ShipCheck은 **아직 사용자가 없는 MVP/프로토타입**을 타겟한다. 이는 제품 라이프사이클에서 완전히 다른 시점이다.

---

## 4. ShipCheck 5-Layer 아키텍처 대비 비교

### Layer별 상세 비교

#### Layer 1: Persona Profile

| 요소 | ShipCheck | Blok | 비교 |
|------|-----------|------|------|
| Demographics | O (나이, 직업, 소득) | △ (행동 클러스터 기반, 개별 인구통계 불명확) | ShipCheck 우위 |
...
| Prior Experience | O (유사 제품 경험 -> 멘탈 모델) | △ (미확인) | ShipCheck 우위 |
...
**Layer 1 판정**: Blok은 실제 사용자 데이터에서 페르소나를 생성하므로 **데이터 기반 정확도**에서 우위. ShipCheck은 **속성의 구조적 깊이와 다양성**에서 우위. 그러나 Blok의 "실데이터 기반"은 기존 사용자가 있어야 한다는 전제조건이 있음.
...
| 요소 | ShipCheck | Blok | 비교 |
|------|-----------|------|------|
| Information Foraging | O | X (미확인) | ShipCheck 우위 |
| Cognitive Load Tracker | O | △ ("cognitive biases" 언급) | ShipCheck 우위 |
| Mental Model Matching | O | X (미확인) | ShipCheck 우위 |
| TAM Score | O | X (미확인) | ShipCheck 우위 |

**Layer 2 판정**: Blok은 "cognitive biases"와 "attention patterns"을 언급하지만, 구조화된 인지 모델이 있는지는 공개 자료에서 확인 불가. ShipCheck의 4가지 인지 이론(Information Foraging, Cognitive Load, Mental Model, TAM) 기반 구조와는 차원이 다를 가능성 높음. **ShipCheck 명확히 우위.**
...
| 요소 | ShipCheck | Blok | 비교 |
|------|-----------|------|------|
| OCC Appraisal | O (22개 감정 카테고리) | X (미확인) | ShipCheck 우위 |
| PAD State | O (3차원 연속 감정 추적) | X (미확인) | ShipCheck 우위 |
| Emotion Decay | O (지수적 감쇠) | X (미확인) | ShipCheck 우위 |
| SDE 확률적 감정 진화 | O (PRISM 차용) | X (미확인) | ShipCheck 우위 |
| Chain-of-Emotion | O (별도 LLM 콜) | X (미확인) | ShipCheck 우위 |

**Layer 3 판정**: Blok의 어떤 공개 자료에서도 감정 시뮬레이션에 대한 언급을 찾을 수 없음. "curious, imperfect and full of nuance"라는 표현은 있으나, 이는 마케팅 언어이지 감정 모델 존재의 증거가 아님. **ShipCheck 명확히 우위.**
...
| 요소 | ShipCheck | Blok | 비교 |
|------|-----------|------|------|
| BDI-E Model | O | X (미확인) | ShipCheck 우위 |
| Fogg Check (B=MAP) | O | X (미확인) | ShipCheck 우위 |
| Satisficing Gate | O | X (미확인) | ShipCheck 우위 |
| Decision Heuristics | O | O ("decision heuristics" 명시) | **동등** |
| PC-POMDP | O (PRISM 차용) | X (미확인) | ShipCheck 우위 |

**Layer 4 판정**: Blok은 "decision heuristics"를 명시적으로 언급하므로 어떤 형태의 의사결정 모델이 있음. 그러나 BDI-E, Fogg, Satisficing 등 구조화된 프레임워크 사용 여부는 미확인. **ShipCheck이 구조적 깊이에서 우위.**
...
| 요소 | ShipCheck | Blok | 비교 |
|------|-----------|------|------|
| Memory Stream | O (Generative Agents 방식) | X (미확인) | ShipCheck 우위 |
| Reflection | O (임계값 기반 추상화) | X (미확인) | ShipCheck 우위 |
| Habit Strength | O (반복 사용 추적) | X (미확인) | ShipCheck 우위 |
| Persona Drift Monitor | O (GenSim 차용) | X (미확인) | ShipCheck 우위 |
| Multi-session | O (Day 1/7/30) | X (미확인) | ShipCheck 우위 |

**Layer 5 판정**: 반복 사용 시뮬레이션이나 메모리 시스템에 대한 언급이 Blok에서는 전혀 없음. **ShipCheck 명확히 우위.**
...
| 차원 | ShipCheck | Blok |
|------|-----------|------|
| **입력** | URL만 (실제 프로덕션 제품) | 이벤트 로그 + Figma 디자인 |
| **시뮬레이션 대상** | 실제 동작하는 웹/앱 | 디자인 목업 + 데이터 모델 |
| **실행 방식** | Playwright 브라우저 자동화 | 행동 모델링 + GenAI 예측 |
| **사용자 데이터 필요** | 불필요 | **필수** |
| **기존 제품 필요** | 필수 (동작하는 MVP) | 불필요 (Figma만으로 가능) |
| **시뮬레이션 범위** | 전체 사용자 경험 (탐색 ~ 이탈) | 특정 실험/변형 비교 |
| **산출물 성격** | 경험 리포트 ("왜 이탈하는가") | 실험 결과 ("어떤 변형이 더 나은가") |
...
**ShipCheck**: Playwright 기반 실제 브라우저 조작. DOM 파싱, 스크린샷, 마이크로 행동 시뮬레이션 (체류 시간, 스크롤, 망설임).

**Blok**: 브라우저 자동화 사용 증거 없음. Figma 디자인 분석 + 이벤트 로그 기반 행동 예측 모델.

### 데이터 통합

**ShipCheck**: 입력 데이터 최소 (URL만). AI가 자체적으로 제품을 크롤링하고 분석.
...
### Q1: Blok이 ShipCheck의 비전을 이미 실현했는가?

**아니다.** Blok과 ShipCheck은 비슷해 보이지만 근본적으로 다른 문제를 풀고 있다.

| 질문 | Blok의 답 | ShipCheck의 답 |
|------|----------|---------------|
...
2. **ShipCheck은 "사용자 경험 시뮬레이션 플랫폼"이다.** AI 페르소나가 실제 제품을 사람처럼 사용하면서 감정/인지/의사결정을 시뮬레이션하는 것이 핵심. 기존 데이터 없이 URL만으로 작동하고, "왜" 특정 경험이 발생하는지를 답하는 것이 목적.

3. **제품 라이프사이클 시점이 다르다:**
   - Blok: 이미 사용자가 있는 제품의 기능 최적화 (Growth 단계)
   - ShipCheck: 아직 사용자가 없는 MVP의 출시 전 검증 (Pre-launch 단계)

### Q2: Blok이 있는데도 ShipCheck을 만들 필요가 있는가?

**있다.** 다음 이유로:

1. **다른 고객, 다른 시점**: Blok의 고객은 Amplitude 데이터가 있는 Growth 팀. ShipCheck의 고객은 아직 첫 사용자가 없는 인디해커. 두 제품은 제품 라이프사이클의 다른 시점을 서빙한다.

2. **다른 질문에 답한다**: Blok은 "어떤 변형이 더 나은가?" ShipCheck은 "이 제품을 쓰면 사람들이 어떤 경험을 하는가?" 전자는 최적화, 후자는 이해.

3. **다른 기술적 접근**: Blok은 데이터 기반 예측 모델. ShipCheck은 실제 브라우저 조작 + 인지/감정 시뮬레이션. 기술 스택이 근본적으로 다르다.

4. **입력 요구사항이 다르다**: Blok은 이벤트 로그 + Figma가 필수. ShipCheck은 URL만. 인디해커의 MVP에는 Amplitude가 붙어있지 않다.

5. **시뮬레이션 깊이가 다르다**: Blok은 행동 예측 수준. ShipCheck은 감정/인지/의사결정까지 시뮬레이션. "왜 이탈하는가"에 대한 답의 깊이가 다르다.

### Q3: "Blok이 있으니 ShipCheck은 필요 없다"가 맞는 판단인가?

**아니다.** 이 판단이 틀린 이유:

1. **Blok은 ShipCheck이 풀려는 문제를 풀지 않는다.** "사용자 데이터가 없는 MVP를 출시 전에 다양한 페르소나로 검증"하는 것은 Blok이 할 수 없다.

2. **Blok의 타겟 세그먼트와 ShipCheck의 비치헤드는 겹치지 않는다.** Blok은 금융/헬스케어 엔터프라이즈. ShipCheck은 인디해커/1인 개발자.

3. **Blok이 증명하는 것은 오히려 ShipCheck에 긍정적이다.** "AI 합성 사용자로 제품을 테스트하는 것"에 $7.5M이 투자되었다는 것은 이 시장에 진짜 수요가 있다는 시장 검증 신호다.

4. **Blok이 해결하지 않는 "경험 이해" 갭이 여전히 존재한다.** 어떤 경쟁자도 "AI 페르소나가 실제 제품을 사용하면서 감정과 인지를 시뮬레이션"하는 것을 하고 있지 않다.

### Q4: ShipCheck이 Blok 대비 취할 수 있는 전략
...
아이디어 → 디자인 → MVP → 첫 사용자 → 성장 → 최적화
              │         │                      │
           Blok 가능   ShipCheck 최적          Blok 최적
           (Figma)     (URL 기반)             (데이터 기반)
```

ShipCheck은 "MVP -> 첫 사용자" 구간에 집중. 이 구간에서 Blok은 작동하지 않음 (데이터 없으므로).

#### 전략 2: 시뮬레이션 깊이 차별화

Blok이 "어떤 변형이 10% 더 나은가"를 답한다면, ShipCheck은 "65세 은퇴자 김 씨가 온보딩 3단계에서 7초간 멍하니 있다가 뒤로가기를 누른 이유"를 답한다. 이 깊이의 차이는 기술적으로 모방하기 어렵다.
...
- 고객 설득 시: "이미 Blok, Aaru 등이 AI 합성 사용자의 유효성을 증명. ShipCheck은 이를 아직 사용자가 없는 MVP에 적용"

#### 전략 5: Blok이 확장할 수 없는 영역 선점

Blok이 "실데이터 기반 행동 예측"에서 "실제 브라우저 조작 + 감정 시뮬레이션"으로 확장하려면:
- 완전히 새로운 기술 스택 구축 필요 (Playwright 등)
- 기존 "데이터 기반 예측" DNA와 충돌
- 기존 고객(엔터프라이즈)의 니즈와 다른 방향
- 가격 모델 재설계 필요 (Self-serve)

이 전환은 Blok에게 쉽지 않다. 반대로 ShipCheck이 "데이터 기반 A/B 테스트 가속"으로 확장하는 것도 어렵지만, ShipCheck은 그럴 필요가 없다 (다른 문제를 풀고 있으므로).

---

## 7. 위험 요인과 모니터링 항목

### Blok이 ShipCheck 방향으로 확장할 가능성
...
> **Blok은 "기존 데이터 기반 A/B 테스트 가속 플랫폼"이고, ShipCheck은 "데이터 없는 MVP의 사용자 경험 시뮬레이션 플랫폼"이다. 비슷해 보이지만 다른 고객, 다른 시점, 다른 질문, 다른 기술로 다른 문제를 푼다.**

### 팩트 기반 판정표

| 판정 항목 | 결론 |
|-----------|------|
| Blok이 ShipCheck 비전을 이미 실현? | **아니다** - 다른 문제를 풀고 있음 |
...
| ShipCheck만의 차별화 여지 있음? | **있다** - 5가지 차원에서 차별화 |
| Blok의 존재가 ShipCheck에 위협? | **부분적** - 장기적 확장 시 경쟁 가능성 |
| Blok의 존재가 ShipCheck에 긍정적? | **그렇다** - 시장 검증, 카테고리 교육 효과 |
