# Value Proposition: ShipCheck

**Date**: 2026-03-17
**Version**: v2.0 — Wave 0 심층 재작성 반영 (커뮤니티 인용, 경쟁사 업데이트, 비용 기반 가격 정당화)
**Input**: docs/34-user-research.md (v2.0), docs/33-lean-canvas.md (v2.0), docs/31-competitive-analysis.md (v2.0), docs/30-market-sizing.md (v2.0), docs/32-market-scan.md (v2.0)
...
> **ShipCheck** is an AI 사용자 시뮬레이션 서비스 that **200명의 AI가 제품을 실제로 사용하고 세그먼트별 피드백을 제공한다**.
> Unlike 지인 테스트나 ChatGPT, **AI가 실제로 제품을 조작한 경험에 기반한 구체적 인사이트를 제공한다**.

### Segment 2 (리서치-없는-PM)

> **For UX 리서처 없는 스타트업 PM** who need 구조화된 사용성 인사이트,
> **ShipCheck** is an AI 사용자 시뮬레이션 서비스 that **200명 AI 페르소나의 세그먼트별 이탈 원인과 감정 궤적을 제공한다**.
...
> **ShipCheck** is an AI 사용자 시뮬레이션 서비스 that **500명 AI 페르소나로 출시 전 세그먼트별 반응을 예측한다**.
...
### Marketing (랜딩페이지, 소셜)
- **헤드라인**: "200명의 AI가 당신 제품을 써보고 솔직히 말해준다"
- **서브**: "Product Hunt 런칭 전, 다양한 사용자가 어디서 막히고 뭘 느끼는지 알아보세요"
- **CTA**: "내 제품 쉽체크 받기"
- **ChatGPT 차별화**: "ChatGPT는 URL만 읽습니다. ShipCheck은 실제로 클릭하고 스크롤하고, 그 경험으로 말해줍니다."

### Sales (PM/CPO 대상)
- "Hotjar는 '어디서 이탈'을 보여줍니다. ShipCheck은 '왜 이탈'을 알려줍니다."
- "유저 인터뷰 5명 x 2주 대신, AI 200명 x 하루. 방향은 같고 속도는 다릅니다."
- "Amplitude 데이터 없이도 동작합니다. URL만 있으면 됩니다."

### Onboarding (가입 후)
- "스테이징 URL과 테스트 계정만 준비하세요. 나머지는 ShipCheck이 합니다."
...
- **투명성 우선**: "ShipCheck은 인간 리서치를 대체하지 않습니다. 인간 리서치 전에 '어디를 깊게 파야 하는지' 방향을 잡아줍니다."
...
- **비교 증명**: "같은 제품에 ChatGPT vs ShipCheck을 돌린 결과를 비교해보세요." — side-by-side 데모
...
Porter Five Forces 분석에서 대체재 위협 강도: **4/5 (High)**. ChatGPT/Claude는 무료이고, "이런 사용자인 척 하고 내 제품 리뷰해줘"가 점점 나아지고 있다. Computer Use/CUA로 스크린샷 기반 분석도 가능해지는 중. 이 질문에 명확히 답하지 못하면 ShipCheck의 존재 이유가 사라진다.

### 구조적 차이

| 차원 | ChatGPT / Claude | ShipCheck |
|------|------------------|-----------|
...
> "ChatGPT는 당신의 제품을 써본 적이 없습니다. ShipCheck의 200명은 실제로 클릭하고 스크롤하고 멍하니 있다가 뒤로 갔습니다."

**상세 반론:**

1. **"상상"과 "경험"의 차이**: ChatGPT에게 "맥주 맛이 어때?"라고 물으면 일반론을 말해줍니다. 실제로 마셔본 사람은 "이 맥주는 첫 모금에 시트러스가 강한데 뒷맛이 쓰다"라고 합니다. ShipCheck은 제품을 "실제로 마셔본" AI입니다.

2. **1명 vs 200명**: ChatGPT는 1명의 관점입니다. 당신이 프롬프트를 바꿔도 LLM의 "평균적 인간" 응답 경향에서 벗어나기 어렵습니다. ShipCheck의 200명은 SDE(확률미분방정식) 기반으로 같은 자극에도 수학적으로 다른 반응을 생성합니다.

3. **의견 vs 데이터**: ChatGPT는 "이 버튼이 직관적이에요"라는 의견을 줍니다. ShipCheck은 "200명 중 47명이 이 버튼을 5초 이상 찾지 못했고, 그중 23명이 뒤로 갔다"는 데이터를 줍니다.
...
ShipCheck의 진짜 차별점은 **브라우저 조작 자체가 아니라, "구조화된 감정/인지/의사결정을 시뮬레이션하면서 브라우저를 조작하는 것"**이다:

- Computer Use는 범용 "작업 수행" 도구. "이 폼 작성해줘"를 잘 한다.
- ShipCheck은 "이 폼을 작성하면서 65세 은퇴자가 어떤 감정을 느끼고, 어디서 인지 부하가 치솟으며, 어느 지점에서 포기 결정을 내리는가"를 시뮬레이션한다.
...
| **ShipCheck** | $0 | — | 인디해커, 초기 스타트업 | **실제 제품 Playwright 조작 + 5-Layer 감정/인지** |

### 왜 "적"이 아닌 "카테고리 교육자"인가

1. **시장 세그먼트가 다르다**: Simile/Aaru는 대기업 대상 엔터프라이즈 세일즈. ShipCheck은 인디해커 대상 셀프서브. 직접 경쟁하는 고객이 없다.

2. **JTBD가 다르다**: Simile은 "인간 행동 예측(디지털 트윈)", Aaru는 "설문/여론 시뮬레이션(포커스그룹 대체)". ShipCheck은 "실제 제품 사용 경험 시뮬레이션(UX 체험 검증)". 풀고 있는 문제가 다르다.

3. **기술 접근이 다르다**: Simile/Aaru/Blok 모두 **실제 제품을 브라우저로 조작하지 않는다.** 전부 Figma/설문/데이터 기반으로 우회. 실제 제품 조작은 ShipCheck이 유일한 상용 서비스다.

4. **$1.5B+가 카테고리를 교육해준다**: Simile의 $100M, Aaru의 $50M+가 "AI 합성 사용자"라는 카테고리를 빠르게 교육하고 있다. 48%의 UX 리서처가 합성 사용자를 "2026년 임팩트 있는 트렌드"로 선정 (Lyssna). Aaru의 EY 파트너십으로 90% 상관관계 결과가 공개되면서 시장 신뢰도가 상승. **ShipCheck이 직접 시장을 교육할 필요가 줄어든다.**

5. **ShipCheck은 교육된 시장 위에 올라탄다**: Aaru/Simile가 수억 달러를 들여 증명한 "합성 리서치가 된다"라는 전제 위에, ShipCheck은 "그런데 실제로 써보는 건 우리뿐이다"를 얹는다.

### 포지셔닝 메시지

- **내부 전략**: "Simile/Aaru가 카테고리를 만들어준다. 우리는 그 카테고리 안에서 '실제 제품 조작'이라는 고유한 포지션을 선점한다."
- **외부 커뮤니케이션**: "AI 합성 사용자 시장에서 $1.5B+가 투자되고 있습니다. 그중 실제 제품을 AI가 사람처럼 사용하는 서비스는 ShipCheck뿐입니다."
- **투자자 피칭**: "Simile($100M), Aaru($1B 밸류에이션)가 검증한 시장입니다. 그러나 모두 Figma/설문/데이터 기반입니다. Pre-launch MVP 시장에서 실제 제품을 브라우저로 조작하는 서비스는 비어 있습니다."
...
ShipCheck의 가격($49-199/회)은 "저렴한 느낌"이 아닌, 실제 원가 구조에서 도출된 건강한 마진 기반 가격이다.
...
1. **LLM 비용 연 80% 하락**: 2024년 GPT-4급 $20/MTok -> 2026년 GPT-4.1 mini $0.40/MTok. 200명 시뮬레이션의 LLM 비용이 $10 수준. 이 비용 하락이 ShipCheck의 비즈니스 모델을 가능하게 한다.

2. **브라우저 자동화 생태계 성숙**: Browserless Usage-based ($0.0048/분)로 200명 시뮬레이션 브라우저 비용이 $4.80. Playwright 1,350만 주간 npm 다운로드 — 인프라가 충분히 성숙.

3. **인간 리서치 대비 10-100x 가격 차이**: UserTesting.com 5명 = $245+ (연 계약 ~$46K/년). ShipCheck 200명 = $99. 이 가격 갭이 가치 제안의 핵심.

### 가격 앵커링 전략

| 비교 대상 | 비용 | ShipCheck | 배율 |
|-----------|------|-----------|------|
...
```
                 ShipCheck  Blok  Uxia  Synthetic  Aaru  Artificial  ChatGPT  UserTesting
                                       Users            Societies            (인간)
실제 제품 사용    *****      *     *     .         .     .           .        *****
감정 시뮬레이션   ****       *     .     **        .     .           *        *****
인지/의사결정    *****       **    .     .         .     .           .        *****
...
**압도적 우위 영역**: 실제 제품 사용 + 감정/인지/의사결정 시뮬레이션 + 반복 사용 시뮬레이션 + 실체험 후 인터뷰 + 행동 로그/마이크로 행동.

**의도적 트레이드오프**: 속도(-) + 세팅 간편성(-)를 감수하고 **깊이**를 선택. Uxia가 5분 만에 결과를 주는 반면, ShipCheck은 1-3시간 걸리지만 "온보딩 3단계에서 65세 페르소나가 CTA를 3초간 못 찾고 뒤로가기를 눌렀다. 인지 부하 0.82, 좌절감 0.71" 수준의 리포트를 제공한다.

**유일한 포지션**: 18개 Feature Matrix (docs/31)에서 "실제 제품 브라우저 조작"을 제공하는 상용 서비스는 ShipCheck뿐이다. 학술 시스템(UXAgent, AgentA/B)만 유사하지만, 감정/인지 시뮬레이션은 ShipCheck에만 있다.

**핵심 약점**: 네트워크/소셜 시뮬레이션은 미지원 (Artificial Societies 영역). 접근성 감사는 Uxia가 WCAG 자동 검사에서 우위. 속도와 세팅 간편성에서 Synthetic Users, Uxia 대비 약세.
...
### 프로젝트 내부 문서
- docs/34-user-research.md — 페르소나 정의 (민수/서연/준혁), 커뮤니티 근거, 이탈 퍼널 분석
- docs/33-lean-canvas.md — 가격 모델, 비용 구조, 실제 API 가격 기반 COGS, 유닛 이코노믹스
- docs/31-competitive-analysis.md — 9개 경쟁사 + 학술 시스템 2개 + 간접 경쟁사 3개, 18개 Feature Matrix
- docs/30-market-sizing.md — TAM $150-300M, SAM $0.5-1M, 경쟁사 펀딩/매출 간접 검증
- docs/32-market-scan.md — SWOT + PESTLE + Porter Five Forces + Ansoff Growth Matrix
