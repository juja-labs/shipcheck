...
| **합성 페르소나 시뮬레이션 플랫폼** | 5-Layer 감정/인지/의사결정 엔진 기반 페르소나 리얼리티 — 제품 체험(Playwright), 서베이, A/B 테스트, 전문가 리뷰, 광고/마케팅 리서치, 퍼널 분석 | **Personica** (유일) |
...
이 시장의 가장 중요한 구조적 특징: **어떤 상용 서비스도 5-Layer 수준의 구조화된 감정/인지/의사결정 시뮬레이션을 제공하지 않는다.** Personica는 이 페르소나 리얼리티를 핵심 해자로 가지며, 경쟁사들이 커버하는 서베이/설문/디자인 기반 영역도 커버하면서 추가로 실제 제품 체험(Playwright) 모드를 제공한다.
...
#### 위협도 평가: **중간-높음**
100,000 페르소나 + 실제 웹 조작 + Amazon 연구 역량. 상용화되면 직접 경쟁. 다만 감정/인지 시뮬레이션 없으므로 Personica의 핵심 차별점(5-Layer)은 유지됨.
...
### 7.1 "5-Layer 페르소나 리얼리티" — 핵심 해자

**설명**: 모든 상용 경쟁사가 구조화된 감정/인지/의사결정 시뮬레이션을 제공하지 않는 상황에서, 5-Layer 엔진의 페르소나 리얼리티(OCC+PAD+SDE 감정, Cognitive Load 인지, BDI-E 의사결정)는 Personica만의 핵심 해자. 이 엔진은 제품 체험(Playwright) 외에도 서베이, A/B 테스트, 전문가 리뷰, 광고/마케팅 리서치, 퍼널 분석 등 모든 모드에서 동일하게 작동한다.
...
**핵심 리스크**: "5-Layer 시뮬레이션 vs 일반 LLM 프롬프팅"의 품질 차이를 정량적으로 증명해야 함. Ablation test로 각 레이어의 기여도를 검증.
...
1. **5-Layer 페르소나 리얼리티(감정/인지/의사결정 시뮬레이션)**: 유일한 차별점. 이 엔진의 가치를 정량적으로 증명하는 것이 최우선
